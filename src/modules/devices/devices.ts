...
import { RootObject, ModemCertificate } from './types/getAllDevices'
import { Requester } from '../../core/helpers/requester'

export class Devices extends ModuleBase {
  private static instance: Devices

  constructor(public requester: Requester) {
    super(requester)
  }

  public static getInstance(config: Requester): Devices {
    if (!Devices.instance) {
      Devices.instance = new Devices(config)
    }
  ...
  /**
   * Retrieve the product certificate associated with a device already registered.w
